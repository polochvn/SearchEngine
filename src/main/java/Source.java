...
public class Source {
    ...
    public ResStatistics getStatistic() {
        ...
        for(Site s : siteList){
            int pages = pageRepository.pagesOnSite(s).size();
            int lemmas = lemmaRepository.lemmasOnSite(s).size();

            s.setPages(pageRepository.pagesOnSite(s));
            s.setLemmas(lemmaRepository.lemmasOnSite(s));
    ...
    public void indexing() {
        ...
        parses.forEach(parse -> threads.add(new Thread(() -> {
            ...
            try {
                ...
                ex.printStackTrace();
                site.setError("Ошибка индексации: " + ex.getMessage());
